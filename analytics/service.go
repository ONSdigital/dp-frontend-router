...
type ServiceBackend interface {
	Store(url, term, listType, gaID string, gID string, pageIndex, linkIndex, pageSize float64)
}
...
func (s *ServiceImpl) CaptureAnalyticsData(r *http.Request) (string, error) {
	...
	if s.backend != nil {
		s.backend.Store(url, term, listType, gaID, gID, pageIndex, linkIndex, pageSize)
	}
